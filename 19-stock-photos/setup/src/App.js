...
function App() {
	...
	const fetchImages = async () => {
		...
		try {
			...
			setPhotos(oldPhotosArray => {
				if (query && page === 1) {
					return data.results;
				} else if (query) {
					return [...oldPhotosArray, ...data.results];
				} else {
					return [...oldPhotosArray, ...data];
				}
			});
	...
	useEffect(() => {
		const eventId = window.addEventListener('scroll', () => {
			if (
				(!loading && window.innerHeight + window.scrollY) >=
				document.body.scrollHeight
			) {
				console.log('we have reached at end!');
				setPage(oldPage => {
					console.log(oldPage);
					return oldPage + 1;
				});
